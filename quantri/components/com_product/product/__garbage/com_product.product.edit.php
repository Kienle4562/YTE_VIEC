...
<?php
	...
	$sql = "SELECT
			  `news`.`News_Id`, `news`.`AccID`, `news`.`Title`, `news`.`Description`,
			  `news`.`Content`, `session`.`Ses_Id`, `category`.`Cat_Id`, `news`.`Status`,
			  `news`.`Focus`, `news`.`alias`, `news`.`img_file`, Date_FORMAT(`news`.`Date`, '%d/%m/%Y') As `date_add`, `news`.`img_status`, `news`.`num_view`
			FROM
			  `session` INNER JOIN
...
?>
		...
		<tr>
			...
			<td>
				<input class="inputbox" type="text" name="title" id="title" size="70" maxlength="255" value="<?php echo $newsTitle ?>" />
			</td>
			<td>
		...
		<tr>
			...
			<td>
				<input class="inputbox" type="text" name="alias" id="alias" size="70" maxlength="255" value="<?php echo $alias ?>" />
			</td>
			<td>
			...
			<?php if($Focus == 1){ ?>
				...
				<label for="frontpage0">Không</label>
			...
			<table width="100%" class="paramlist admintable" cellspacing="1">
				<tr>
					...
					<td class="paramlist_value">
						<SELECT class=inputbox size=1 name=created_by class="inputbox">
							<?php include("../protected/dbconnect.php");
