	...
	if ($.inArray(fileType, ValidImageTypes) < 0) {
		...
		$.ajax({
			...
			success: function(result){
				...
				$('input[name=' + elm.replace("file_", "") + ']').val(result.trim());
	...
	$.getJSON('Model_'+com_name+'.ajax?id='+id+"&act=LoadDataEdit", function(data) {
		for (var i in data) {
			...
			switch(type){
				...
				case 'radio':
					$el.removeAttr('checked');
					$el.filter('[value="'+data[i]+'"]').attr('checked', 'checked');
				break;
				case 'hidden':
					if(data[i].length == 0){
						data[i] = "image/noimage.jpg";
...
function updateData(event, com_name, elm){
	...
	if($("form")[0].checkValidity()) {
		$("#m_form .currency").each(function(){
			$(this).val($(this).val().replace(/,/g,""));
		})
		
		var valueCheckBox = "";
		$("#m_form input[type=checkbox]:not(:checked)").each(function(){
			valueCheckBox += '&'+this.name+'=0';
		})
		
		$.ajax({
			url: "Model_"+com_name+".ajax?act=Update",
			type: "POST",
			data: $("#m_form").serialize() + valueCheckBox,
			beforeSend: function(){
			...
			success: function(result){
				setTimeout(function() {
					...
					if(result == 1){
						if($('#m_form .currency').length > 0){
							$("#m_form .currency").val(format($("#m_form .currency").val()));
						}
						toastr["success"]("Đã cập nhật!", "Cập nhật dữ liệu thành công!");
					}else{
						toastr["warning"]("Lỗi", "Xin kiểm tra lại");
		...
		$("#m_form").find(":invalid").addClass('required').first().focus();
		$("#m_form").find(":invalid").each(function(index, node) {
