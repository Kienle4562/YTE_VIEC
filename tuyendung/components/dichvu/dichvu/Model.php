<?php defined( '_VALID_MOS' ) or die( include("404.php") );
    class process
    {
		...
		function Update_donhang_detail($trang_thai,$ngayhethan,$UPDATE_DATE,$customer_detail_id)
				{	
				  ...
				  if ($this->dbObj->SqlQueryInputResult($sql,array($trang_thai,$ngayhethan,$UPDATE_DATE,$customer_detail_id))<> FALSE) 
					  {
						 return true;
					  } else {
						 return false;
		...
		public function get_order_detail($id)
        {
            $sql = "
				SELECT
					...
					trn_customer.`status`,
					DATE_FORMAT(ngay_het_han, '%d/%m/%Y') AS ngayhethan,
					DATE_FORMAT(ngay_kich_hoat, '%d/%m/%Y') AS ngaykichhoat,
...

        {
...

            {
...

                {
                    ...
                    $order_detail_sql = "INSERT INTO `trn_customer_detail` (customer_id,service_id,name_function,function_id,qty,don_gia,giam_gia,kieu_giam,thanh_tien,trang_thai,so_ngay_sd,INSERT_DATE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
...

                    {
                        ...
                        $this->dbObj->SqlQueryInputResult(
                            ...
                            $order_detail_sql,
							array($new_order['customer_id'],$row['service_id'],$row['code_function'],$row['id_function'],$qty,$row['service_price'],$giamgia_pt,$kieugiam,$total,0,$row['operation'],$INSERT_DATE)
    ...
    */
    ...
    switch(@$_REQUEST["act"]){
		...
		case 'payment-process':
			 ...
			 break;
	    case 'checkApply-activer':
		//	var_dump($_POST);
			$myprocess = new process();
			$myObj = new \stdClass();
			$customID  = $_POST['customerID'];
			$dateExt =$core_class->_formatdate( $_POST['dateExt']);
			$codefunc= $_POST['code'];
			$point = $_POST['point'];
			$idcty = $_POST['idcty'];
			$activation_date = date('Y-m-d H:i:s');
			if($myprocess->Update_donhang_detail(1,$dateExt,$activation_date,$customID))
			{
				if($codefunc == 'search_cv')
				{
					if($myprocess->Update_congcty($point,$idcty))
					{
						$myObj->isError = 0;
						$myObj->msg = 'Kích hoạt thành công';
						$myObj->focus = '';
						$myJSON = json_encode($myObj);
						echo $myJSON;
						exit();
