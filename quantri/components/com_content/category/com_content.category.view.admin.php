<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
	...
	if (!isset($_SESSION['amdin']['com_content']['category']['lang_code']))
	{
		$_SESSION['amdin']['com_content']['category']['lang_code'] = 'vi';
	}
...
<div id="content-box">
		<div class="border">
			...
			<div class="header icon-48-sections">Quản lý danh mục bài viết</div>
						...
						<form method="post" name="phpFormTop">
							...
							<select name="lang_code" onchange="document.phpFormTop.submit()">
								<?php
									...
									{
										if (isset($_SESSION['amdin']['com_content']['category']['lang_code']) && $_SESSION['amdin']['com_content']['category']['lang_code'] == $key)
										{
			...
			<div class="m">
				<form method="post" name="phpForm">
					...
					<?php
						...
						{
							global $myprocess;
							$result = $myprocess->get_category_view($parentid, $_SESSION['amdin']['com_content']['category']['lang_code']);
							$k = 1;
					...
					?>
						<tr>
							...
							<?php } ?>
							<th class="title" style="width:60%"><a href="#">Danh mục</a></th>
					...
					?>
						<tr class="row<?php if ($i % 2 == 0) { echo "0"; } else { echo "1"; } ?>">
							...
							<?php } ?>
							<td nowrap="nowrap">
                                <span class="editlinktip hasTip" title="Nhấp chuột để sửa đổi danh mục tin tức này::<?php echo $category[$i]['title']; ?>">
									...
									<a href=".?com=com_content&view=category&task=edit&id=<?php echo $category[$i]['id']; ?>"><?php echo $category[$i]['title']; ?></a>
								</span>
							</td>			
							<td align="center">			
								<?php
