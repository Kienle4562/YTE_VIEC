								    ...
								    if($rs['stt'] > $pager->offset && $rs['stt'] <= $pager->limit){ ?>
									    <tr class="row<?php if($rs['stt'] % 2 == 0){echo "0";}else{ echo "1";}?>">
										    ...
										    <td>
											    ...
											    <strong>&nbsp;&nbsp;|____ </strong> &nbsp;
											    <a style="cursor:pointer" onClick="window.parent.jSelectArticle(<?php echo "'" . $rs['category_id'] . "', '" . $rs['alias'] . "/n" . $rs['id'] . "', '" . str_replace("'", "\\'", $rs['news_title']) . "'"; ?>,'<?= $_REQUEST["khung"] ?>')" href="javascript:void(0)">
