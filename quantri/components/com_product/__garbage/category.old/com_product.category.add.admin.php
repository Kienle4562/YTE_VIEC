...
<div class="header icon-48-sections">Sản phẩm » <small>Danh mục » </small><small><small>Thêm mới</small></small></div>
					...
					<td colspan="2">
						<SELECT class=inputbox size=1 name=sectionid>
						  ...
						  <?php
							...
							$sql = "Select `Id`, `session_name` From `book_session` order by `order_num` desc";
							...
							*/
							...
							if(isset($_GET["id"])){
								while($row = $result->fetch())
								{
									extract($row);
									if($Id == $_GET["id"]){
										echo "<OPTION selected value=$Id>$session_name</OPTION>";
									} else {
										echo "<OPTION value=$Id>$session_name</OPTION>";
									}
								}
							} else {
								echo "<OPTION value=$Id>$session_name</OPTION>";
