<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
    ...
    class process_mod_product extends modules_models
    {
		public function get_cate_alias($id){
			$sql = "select alias from product_category where cat_id=?";
        ...
        public function get_category_list($id, $delim = ' » ', $col = 'title', $lang_code = 'vi')
        {
		 ...
		 public function category_multi_level_2($parentid, $lang_code)
        {
            $sql = "
                    SELECT
                            `cat_id`,
		...
		/*product hot*/
		public function category_multi_level_hot($parentid, $lang_code)
        {
            $sql = "
                    SELECT
                            `cat_id`,
        ...
        function get_category_hot($cat_id, $limit, $lang_code)
        {
            $limit_query = "";
            if (is_numeric($limit) && $limit > 0) {
                $limit_query = "LIMIT 0," . $limit;
        ...
        public function get_category_list_hot($id, $delim = ' » ', $col = 'title', $lang_code = 'vi')
        {
            $cond = '';
            if (is_numeric($id)) {
                $cond = '`cat_id` = ?';
