<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
    ...
    class process extends com_product_category
    {
	    ...
	    function process_edit_category($parent_id, $title, $alias, $description, $image, $date_add, $date_modify, $enabled, $id, $display_type)
	    {
		    $sql = " Update product_category SET
						...
						`display_type` = ?
				    WHERE cat_id = ?";
		    if($this->dbObj->SqlQueryInputResult($sql, array($parent_id, $this->dbObj->fix_quotes_dquotes($title), $alias, $description, $image, $date_add, $date_modify, $enabled, $display_type, $id)) <> FALSE){
			    return true;
		    }
	    }
	    ...
	    {
		    $sql = "SELECT `cat_id`, `parent_id`, `title`, `alias`, `description`, `image`, `date_add`, `enabled`, `lang_code`, `display_type`
    ...
    */
    ...
    switch($_POST["hidden"])
    {
        ...
        case "submit_com_product_category_edit";
            ...
            if ($_POST["task"] == "save")
            {
                if ($myprocess->process_edit_category( $parent, $title, $alias, $description, $image[0], $date_add, $date_modify, $published, $cid, $display_type) <> FALSE)
                {
                	if ($display_type == -1) {
