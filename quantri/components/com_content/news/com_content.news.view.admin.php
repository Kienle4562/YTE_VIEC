...
if($_SESSION["session"]["key"] == "Supper Administrator" || $_SESSION["session"]["key"] == "Administrator"){ 
	...
	if (!isset($_SESSION['amdin']['com_content']['news']['lang_code']))
	{
		$_SESSION['amdin']['com_content']['news']['lang_code'] = 'vi';
	}
	?>
...
<script type="text/javascript">	
	...
	jQuery(document).ready(function () {
		...
		{
			jQuery('.split_panel').css('height', jQuery('.ui-layout-center').height() + 50);
		}
...
<script language="javascript">
	function filter_data(){
		...
		if(start_date != "" && from_date != ""){
			location.href = ".?com=com_content&view=news&task=view&catid=" + catid + "&authorid=" + authorid + "&start_date=" + start_date + "&from_date=" + from_date;
		} else {
			location.href = ".?com=com_content&view=news&task=view&catid=" + catid + "&authorid=" + authorid;
		}
	}
</script>
<div id="content-box">
		...
		<div class="padding">
							...
							<tbody>
								<tr>
									...
									<td class="button" id="toolbar-new">
										<a href=".?com=com_content&view=news&task=add" class="toolbar">
			...
			<div id="element-box" style="margin-bottom:10px;">
				...
				<div class="m">
					...
					<strong>
						<a href=".?com=com_content&view=category&task=view">Danh mục</a> | 
						<a href=".?com=com_content&view=news&task=view">Bài viết</a>
					</strong>
							...
							<form method="post" name="phpFormTop">
								...
								<select name="lang_code" onchange="document.phpFormTop.submit()">
									<?php
										...
										{
											if (isset($_SESSION['amdin']['com_content']['news']['lang_code']) && $_SESSION['amdin']['com_content']['news']['lang_code'] == $key)
											{
								...
								<div id="sidetreecontrol"><a href="?#">Collapse All</a> | <a href="?#">Expand All</a></div> 
									...
									<?php
										...
										function categories ($parentid = 0)
										{
										    ...
										    $result = $myprocess->category_multi_level($parentid, $_SESSION['amdin']['com_content']['news']['lang_code']);
											...
											if ($total > 0) {
                                                ...
                                                while ($row = $result->fetch())
                                                {
                                                    ...
                                                    if ($parentid == 0) {
														echo '<a href="?com=com_content&view=news&task=view&catid=', $row["cat_id"], '"><strong>', $core_class->create_lang_flag($row['lang_code'], 16, true), ' ', $row["title"], '</strong></a>';
													} else {
														echo '<a href="?com=com_content&view=news&task=view&catid=', $row["cat_id"], '">', $core_class->create_lang_flag($row['lang_code'], 16, true), ' ', $row["title"], '</a>';
													}
								...
								<thead>
									...
									<?php 
                                        ...
                                        if( isset($_GET["catid"]) && $_GET["catid"] != 0) {
                                            // $catid = " AND find_in_set(`news`.`category_id`, (SELECT `catid_array` FROM `category` WHERE `category`.`cat_id` = " . intval($_GET["catid"]) .")) "; 
                                            $catid = "
                                                    ...
                                                    FROM
                                                            ...
                                                            (
                                                                ...
                                                                WHERE
                                                                        ...
                                                                        AND `lang_code` = '" . $_SESSION['amdin']['com_content']['news']['lang_code'] . "'
                                                                LIMIT 0,1
                                                            ) as a
                                                    WHERE `category`.`left` >= a.`left` AND `category`.`right` <= a.`right`
                                        ...
                                        if (isset($_GET["authorid"]) && $_GET["authorid"] != 0) {
                                            $authorid = " AND `account`.`Ac_Id` = " . intval($_GET["authorid"]);
                                        }
										
										$conditions = $catid . $authorid;
										$result = $myprocess->get_article_view($conditions, $_SESSION['amdin']['com_content']['news']['lang_code']);
								...
								<tbody>
									...
									foreach($table_result as $k=>$rs) {
										if($rs['stt'] > $pager->offset && $rs['stt'] <= $pager->limit){ ?>
											<tr class="row<?php echo $rs['stt'] % 2; ?>">
												<td align="center"><?php echo $rs['stt'] ?></td>
												<td align="center"><?php echo $rs['id'] ?></td>
												<td>
													<div style="padding-bottom:8px;">
														<a href=".?com=com_content&view=news&task=view&catid=<?php echo $rs['category_id'] ?>">
