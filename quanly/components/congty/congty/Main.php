		...
		<div class="m-portlet m-portlet--mobile">
			<div class="m-portlet__head">
				...
				<div class="m-portlet__head-tools">
					...
					<button onclick="showDialogInsert()" class="btn headTool  btn-danger m-btn m-btn--custom m-btn--icon m-btn--air" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#Dialog_ThemMoi">
								<span>
							...
							<?php if($_SESSION["session"]['Id'] == 4) { ?>
								...
								<div class="m-form__control">
									<select id="fillterUser" class="form-control m-bootstrap-select m_selectpicker" data-live-search="true">
										...
										<?php
											foreach($row as $key){
												if($_REQUEST["user"] == $key['taikhoan_id']){
													$selected = 'selected="selected"';
												}else{
													$selected = "";
												}
												echo '<option '.$selected.' value="'.$key['taikhoan_id'].'">'.$key['Tendangnhap'].'</option>';
											}
										?>
									</select>
								</div>
...
<div class="modal fade" id="Dialog_ThemMoi" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-slg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">THÊM MỚI THÔNG TIN CÔNG TY</h5>
	...
	<div class="modal fade" id="Dialog_Copy" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-slg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">SAO CHÉP THÔNG TIN CÔNG TY</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
		...
		columns: [
			...
			,{
				...
				template: function (row, index, datatable) {
					var srchinh = row.hinhanh == "" ? "image/noimage.jpg" : row.hinhanh;
					return '<img height=60 src="'+srchinh+'">';
				}
			} */, {
				field: 'tencongty',
				title: 'Tên công ty',
