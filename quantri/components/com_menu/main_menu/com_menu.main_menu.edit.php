...
<div id="content-box">
	<div class="border">
		...
		<div class="m">
			...
			<div class="header icon-48-menumgr">Menus » <small>Chỉnh sửa</small></div>
...
<?php
	...
	$result = $myprocess->get_main_menu_edit(intval($_GET["id"]));
	if($row = $result->fetch()){
?>
<form method="post" name="phpForm">
	<div>
		...
		<legend>Thông tin menu</legend>
			...
			<tr>
				...
				<td>
					<input type="text" name="title" id="title" class="inputbox" size="40" value="<?php echo $row["title"]; ?>" autocomplete="off" /> 
					...
					<select name="lang_code">
						<?php
							if ($row['isroot'] != 1)
							{
								...
								{
									echo 	'<option value="', $key, '"', (($key == $row['lang_code']) ? ' selected="selected"' : ''), '>',
												$item['lang_name'],
											'</option>';
								}
							}
							...
							{
								echo '<option value="vi">Tiếng Việt</option>';
							}
						?>
					</select>
				</td>
			...
			<tr>
				...
				<td>
					<input type="text" name="alias" id="alias" class="inputbox" size="40" value="<?php echo $row["alias"]; ?>" /> 
			...
			<tr>
				...
				<td>
					...
					<input type="radio" name="published" id="published0" value="0" <?php if($row["activated"] == 0){ echo 'checked="checked"'; }?> />
					<label for="published1">Yes</label>
					<input type="radio" name="published" id="published1" value="1" checked="checked" <?php if($row["activated"] == 1){ echo 'checked="checked"'; }?> />
				</td>
			</tr>
			</table>
		</fieldset>
	...
	<input type="hidden" name="hidden" value="submit_com_menu_main_menu_edit" />
	<input type="hidden" name="menu_type_id" value="<?php echo $row["Id"]; ?>"/>
