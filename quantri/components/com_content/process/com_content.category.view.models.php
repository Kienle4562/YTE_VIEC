<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
    ...
    class process extends com_content_category
    {
	    ...
	    {
		    $sql = "SELECT
				      `category`.`cat_id`, `category`.`title`, `category`.`date_add`, `category`.`enabled`, `category`.`ordering`, `category`.`num_view`, `category`.`catid_array`, `category`.`left`, `category`.`right`, `category`.`lang_code`
				    FROM `category`
				    WHERE parent_id = ? and `lang_code` = ? order by `ordering`;";
	    ...
	    function process_pulish_and_un_publish_category($check, $values){
		    if($check == 0)
		    $sql = "Update category Set `enabled` = 0 Where cat_id = ?";
		    else $sql = "Update category Set `enabled` = 1 Where cat_id = ?";
	    ...
	    function process_order_down_category($pnewsid, $lang_code)
	    {
		    ...
		    if($row = $result->fetch()){
			    $sql1 = "update category set `ordering` = ? where `cat_id` = ?";
			    $result1 = $this->dbObj->SqlQueryInputResult($sql1, array($row['currenOrder'], $row['preSesid']));
			    if($result1 > 0){
				    $sql2 = "update category set `ordering` = ? where `cat_id` = ?";
	    ...
	    function process_remove_category($values)
	    {
		    ...
		    $result = $this->dbObj->SqlQueryOutputResult("
		    	...
		    	SELECT
		    			GROUP_CONCAT(`category`.`cat_id`) as `id_list`
		    	FROM
		    			`category`,
		    			(SELECT `left`,`right` FROM `category` WHERE `cat_id` = ?) as A
		    	WHERE
		    ...
		    if ($row = $result->fetch(PDO::FETCH_ASSOC))
		    {
		    	...
		    	{
		    		$sql = "
		    				DELETE FROM `news` WHERE `category_id` IN ({$row['id_list']});
		    				
		    				DELETE FROM `category` WHERE `cat_id` IN ({$row['id_list']});
		    		";
	    ...
	    {
		    $sql = "SELECT count(cat_id) 'count' FROM `category` WHERE parent_id = ?";
	    ...
	    */
	    function copy_category_update(&$sql_union)
	    {
			...
			while ($row_update = $result_update->fetch(PDO::FETCH_ASSOC))
			{
				...
				$sql_update = "UPDATE `category` SET `parent_id` = " . $row_update['parent_id'] . " WHERE `cat_id` = " . $row_update['cat_id'] . ";";
	    ...
	    function copy_category()
	    {
	    	...
	    	foreach ($GLOBALS['LANG_LIST'] as $key => $item)
	    	{
	    		if ($key != 'vi')
	    		{
					$this->dbObj->SqlQueryInputResult("INSERT IGNORE INTO `category`(`lang_code`, `parent_id`, `catid_array`, `root`, `title`, `alias`, `description`, `date_add`, `date_modify`, `ordering`, `enabled`, `num_view`, `left`, `right`, `assoc_id`) (SELECT '" . $key . "' as `lang_code`, -2 as `parent_id`, `catid_array`, `root`, CONCAT(`title`, ' (Sao chép lúc ', CURDATE(), ' ', CURTIME(), ')'), `alias`, `description`, `date_add`, `date_modify`, `ordering`, 0 as `enabled`, `num_view`, `left`, `right`, `cat_id` as `assoc_id` FROM `category` WHERE `lang_code` = 'vi' AND `cat_id` NOT IN (SELECT `assoc_id` FROM `category` WHERE `lang_code` = '" . $key . "'));", array());
				}
			}

			$result = $this->dbObj->SqlQueryOutputResult("SELECT `lang_code`, `cat_id`, `assoc_id` FROM `category` WHERE `parent_id` = -2", array());
			...
			if ($result->rowCount() > 0)
			{
				...
				{
					$sql_single = "UNION (SELECT '{$row['lang_code']}' as `lang_code`, '{$row['cat_id']}' as `cat_id`, R2.`cat_id` as `parent_id` FROM `category` as R2 WHERE R2.`assoc_id` = (SELECT R1.`parent_id` FROM `category` as R1 WHERE R1.`cat_id` = {$row['assoc_id']}) AND R2.`lang_code` = '{$row['lang_code']}') ";
				...
				$this->dbObj->SqlQueryInputResult('UPDATE `category` SET `parent_id` = 0 WHERE `parent_id` = -2', array());
			}
    ...
    switch($_POST["hidden"])
    {
        ...
        case "submit_com_content_category_view";
            if($_POST["task"] == "unpublish"){
                ...
                $myprocess = new process;
                for ($row = 0; $row < count($values); $row++){
                    if($myprocess->process_order_up_category($values[$row], $_SESSION['amdin']['com_content']['category']['lang_code']) <> FALSE)
                    $GLOBALS['msg'] = "";
                    else $GLOBALS['msg'] = "Hiện tại hệ thống đang gặp lỗi, vui lòng liên hệ quản trị !";
                }            
            }
                ...
                $myprocess = new process;
                for ($row = 0; $row < count($values); $row++){
                    if($myprocess->process_order_down_category($values[$row], $_SESSION['amdin']['com_content']['category']['lang_code']) <> FALSE)                    
                    $GLOBALS['msg'] = "";
                    else $GLOBALS['msg'] = "Hiện tại hệ thống đang gặp lỗi, vui lòng liên hệ quản trị !";
                }
            }
                ...
                $core_class->_redirect(".?com=com_content&view=category&task=add");
                exit;
            }
				...
				$_SESSION['amdin']['com_content']['category']['lang_code'] = $_POST['lang_code'];
				$core_class->_redirect('?com=com_content&view=category&task=view');
				exit();
            }
        break;
