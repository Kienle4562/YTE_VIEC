	    ...
	    */
	    ...
	    function copy_category()
	    {
	    	...
	    	foreach ($GLOBALS['LANG_LIST'] as $key => $item)
	    	{
	    		if ($key != 'vi')
	    		{
					$this->dbObj->SqlQueryInputResult("INSERT IGNORE INTO `product_category`(`lang_code`, `parent_id`, `catid_array`, `root`, `title`, `alias`, `description`, `date_add`, `date_modify`, `ordering`, `enabled`, `num_view`, `left`, `right`, `assoc_id`, `display_type`) (SELECT '" . $key . "' as `lang_code`, -2 as `parent_id`, `catid_array`, `root`, CONCAT(`title`, ('" . $key . "')), `alias`, `description`, `date_add`, `date_modify`, `ordering`, 0 as `enabled`, `num_view`, `left`, `right`, `cat_id` as `assoc_id`, `display_type` FROM `product_category` WHERE `lang_code` = 'vi' AND `cat_id` NOT IN (SELECT `assoc_id` FROM `product_category` WHERE `lang_code` = '" . $key . "'));", array());
				}
			}
