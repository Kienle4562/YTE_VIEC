 ...
 * jQueryDate: 2011-06-25 08:00:00 (Sat, 25 Jun 2011) jQuery
 * jQueryRev: 302915 jQuery
 *
 * Copyright (c) 2010 
...
,	language: {
	...
	// can update code here if jQuery.browser is phased out
,	browser: {
		mozilla:	!!jQuery.browser.mozilla
	,	webkit:		!!jQuery.browser.webkit || !!jQuery.browser.safari // webkit = jQ 1.4
	...
	//,	version:	jQuery.browser.version - not used
	}
...
,	getScrollbarSize:	function (dim) {
		...
		var d	= { width: jQueryc.width() - jQueryc[0].clientWidth, height: jQueryc.height() - jQueryc[0].clientHeight };
		jQueryc.remove();
		window.scrollbarWidth	= d.width;
		window.scrollbarHeight	= d.height;
		return dim.match(/^(width|height)jQuery/i) ? d[dim] : d;
	}
...
,	showInvisibly: function (jQueryE, force) {
		...
		var CSS = {
			display:	jQueryE.css('display')
		,	visibility:	jQueryE.css('visibility')
		};
		if (force || CSS.display == "none") { // only if not *already hidden*
			jQueryE.css({ display: "block", visibility: "hidden" }); // show element 'invisibly' so can be measured
			return CSS;
		}
		else return {};
	}
	...
	*/
,	getElementDimensions: function (jQueryE) {
		var
			d	= {}			// dimensions hash
		,	x	= d.css = {}	// CSS hash
		,	i	= {}			// TEMP insets
		...
		d.offsetWidth	= jQueryE.innerWidth();
		d.offsetHeight	= jQueryE.innerHeight();
...
,	getElementCSS: function (jQueryE, list) {
		var
			CSS	= {}
...
,	cssWidth: function (jQueryE, outerWidth) {
		...
		if (!jQuery.layout.browser.boxModel) return outerWidth;

		// strip border and padding from outerWidth to get CSS Width
		var W = outerWidth
			- b(jQueryE, "Left")
			- b(jQueryE, "Right")
			- n(jQueryE, "paddingLeft")		
			- n(jQueryE, "paddingRight")
		;
...
,	cssHeight: function (jQueryE, outerHeight) {
		...
		if (!jQuery.layout.browser.boxModel) return outerHeight;

		// strip border and padding from outerHeight to get CSS Height
		var H = outerHeight
			- b(jQueryE, "Top")
			- b(jQueryE, "Bottom")
			- n(jQueryE, "paddingTop")
			- n(jQueryE, "paddingBottom")
		;
...
,	cssNum: function (jQueryE, prop, allowAuto) {
		...
		,	p	= jQuery.curCSS(jQueryE[0], prop, true)
...
,	borderWidth: function (el, side) {
		...
		return jQuery.curCSS(el, b+"Style", true) == "none" ? 0 : (parseInt(jQuery.curCSS(el, b+"Width", true), 10) || 0);
	}
 ...
 */
	...
	lang = jQuery.layout.language // internal alias
...
,	str = function (o) { // trim converts 'String object' to a simple string
		return isStr(o) ? jQuery.trim(o) : o == undefined || o == null ? "" : o;
	}
...
,	_transformData = function (d) {
		...
		if (d.effects || d.cookie || d.defaults || d.north || d.south || d.west || d.east || d.center)
			json = jQuery.extend( true, json, d ); // already in json format - add to base keys
		else
...
,	_queue = function (action, pane, param) {
		...
		jQuery.each(_c.borderPanes.split(","), function (i, p) {
			if (_c[p].isMoving) {
		...
		function bindCallback (p) {
			...
			if (!c.doCallback) {
				...
				if (cbPane != pane && !jQuery.inArray(cbPane, tried) >= 0)
...
,	_execCallback = function (pane, v_fn) {
		...
		try {
			...
			if (typeof fn=="function") {
				if (pane && jQueryPs[pane])
					// pass data: pane-name, pane-element, pane-state, pane-options, and layout-name
					return fn( pane, jQueryPs[pane], state[pane], options[pane], options.name );
...
,	_fixIframe = function (pane) {
		if (jQuery.layout.browser.mozilla) return; // skip FireFox - it auto-refreshes iframes onShow
...
,	cssW = function (el, outerWidth) {
		var	str	= isStr(el)
		,	jQueryE	= str ? jQueryPs[el] : jQuery(el)
		;
		if (!jQueryE.length) return 0;
		if (isNaN(outerWidth)) // not specified
			outerWidth = str ? getPaneSize(el) : jQueryE.outerWidth();
		return jQuery.layout.cssWidth(jQueryE, outerWidth);
	}
...
,	cssH = function (el, outerHeight) {
		...
		if (isNaN(outerHeight)) // not specified
			outerHeight = str ? getPaneSize(el) : jQueryE.outerHeight();
		return jQuery.layout.cssHeight(jQueryE, outerHeight);
	}
...
,	setOuterWidth = function (el, outerWidth, autoHide) {
		...
		if (w > 0) {
			if (autoHide && jQueryE.data('autoHidden') && jQueryE.innerHeight() > 0) {
				...
				if (!jQuery.layout.browser.mozilla) // FireFox refreshes iframes - IE does not
...
,	_parseSize = function (pane, size, dir) {
		...
		if (size === 0)
			...
			,	s	= jQueryP.css(dim); // SAVE current size
			;
			jQueryP.css(dim, "auto");
			size = (dim == "height") ? jQueryP.outerHeight() : jQueryP.outerWidth(); // MEASURE
			jQueryP.css(dim, s).css(vis); // RESET size & visibility
			return size;
		}
	}
...
,	calcNewCenterPaneDims = function () {
	...
	* @return JSON  Returns a hash of all dimensions: top, bottom, left, right, outerWidth, innerHeight, etc
	*/
,	elDims = function (jQueryE) { return jQuery.layout.getElementDimensions(jQueryE); }

,	elCSS = function (jQueryE, list) { return jQuery.layout.getElementCSS(jQueryE, list); }
...
,	_create = function () {
		...
		jQuery.layout.browser.boxModel = jQuery.support.boxModel;
...
,	_initLayoutElements = function () {
		...
		jQuery.extend(sC, elDims( jQueryN ));
		...
		if (o.resizeWithWindow && !jQueryN.data("layoutRole")) // skip if 'nested' inside a pane
...
,	_initContainer = function () {
		...
		if (!jQueryN.data("layoutCSS")) {
			...
			if (fullPage) {
				...
				// ALSO SAVE <HTML> CSS
				var jQueryH = jQuery("html");
				jQueryH.data("layoutCSS", {
				...
				CSS = elCSS(jQueryN, props+",top,bottom,left,right,width,height,overflow,overflowX,overflowY");

			jQueryN.data("layoutCSS", CSS);
		}
		...
		try { // format html/body if this is a full page layout
			if (fullPage) {
				...
				var
					p = jQueryN.css("position")
				,	h = jQueryN.css("height")
				;
				// if this is a NESTED layout, then container/outer-pane ALREADY has position and height
				if (!jQueryN.data("layoutRole")) {
...
,	initHotkeys = function (panes) {
		...
		jQuery.each(panes.split(","), function (i, pane) {
			var o = options[pane];
...
,	initOptions = function () {
		...
		jQuery.each(_c.allPanes.split(","), function (i, pane) {
			renameOpts(opts[pane]);
		});

		// update default effects, if case user passed key
		if (opts.effects) {
			jQuery.extend( effects, opts.effects );
			delete opts.effects;
		}
		jQuery.extend( options.cookie, opts.cookie );
		...
		jQuery.each(globals.split(","), function (i, key) {
		...
		jQuery.extend( options.center, opts.center );
		// Most 'default options' do not apply to 'center', so add only those that DO
		var o_Center = jQuery.extend( true, {}, options.defaults, opts.defaults, options.center ); // TEMP data
		...
		jQuery.each(_c.borderPanes.split(","), function (i, pane) {

			// apply 'pane-defaults' to CONFIG.[PANE]
			_c[pane] = jQuery.extend( true, {}, _c.panes, _c[pane] );

			// apply 'pane-defaults' +  user-options to OPTIONS.PANE
			o = options[pane] = jQuery.extend( true, {}, options.defaults, options[pane], opts.defaults, opts[pane] );
			...
			jQuery.each(["_open","_close",""], function (i,n) { 
				var
					sName		= "fxName"+n
				,	sSpeed		= "fxSpeed"+n
				...
				var fxName = o[sName];
				if (fxName == "none" || !jQuery.effects || !jQuery.effects[fxName] || (!effects[fxName] && !o[sSettings] && !o.fxSettings))
...
,	initPanes = function () {
		...
		jQuery.each(_c.allPanes.split(","), function (idx, pane) {
			addPane( pane, true );
		});
		...
		if (options.showErrorMessages && jQueryN.innerHeight() < 2)
			alert( lang.errContainerHeight.replace(/CONTAINER/, sC.ref) );
	}
...
,	addPane = function (pane, force) {
		...
		if (!jQueryP.data("layoutCSS")) {
			...
			jQueryP.data("layoutCSS", elCSS(jQueryP, props));
		}
...
,	initHandles = function (panes) {
		...
		jQuery.each(panes.split(","), function (i, pane) {
			...
			,	_state	= (s.isVisible ? "-open" : "-closed") // used for classNames
				// INIT RESIZER BAR
			,	jQueryR		= jQueryRs[pane] = jQuery("<div></div>")
				// INIT TOGGLER BUTTON
			,	jQueryT		= (o.closable ? jQueryTs[pane] = jQuery("<div></div>") : false)
			;

			//if (s.isVisible && o.resizable) ... handled by initResizable
			if (!s.isVisible && o.slidable)
				jQueryR.attr("title", o.sliderTip).css("cursor", o.sliderCursor);
...
,	initContent = function (pane, resize) {
		...
		,	jQueryC
		;
		if (sel) jQueryC = jQueryCs[pane] = (o.findNestedContent)
		...
		if (jQueryC && jQueryC.length) {
			...
			if (!jQueryC.data("layoutCSS"))
				jQueryC.data("layoutCSS", elCSS(jQueryC, "height"));
...
,	initButtons = function () {
		...
		jQuery.each("toggle,open,close,pin,toggle-slide,open-slide".split(","), function (i, action) {
			jQuery.each(_c.borderPanes.split(","), function (ii, pane) {
...
,	initResizable = function (panes) {
		var
			draggingAvailable = (typeof jQuery.fn.draggable == "function")
		,	jQueryFrames, side // set in start()
		;
		if (!panes || panes == "all") panes = _c.borderPanes;

		jQuery.each(panes.split(","), function (idx, pane) {
			var 
				o	= options[pane]
			,	s	= state[pane]
			,	c	= _c[pane]
			...
			jQueryR.bind("mouseenter."+ sID, onResizerEnter)
			...
			,	start: function (e, ui) {
					...
					jQueryFrames = jQuery(o.maskIframesOnResize === true ? "iframe" : o.maskIframesOnResize).filter(":visible");
					var id, i=0; // ID incrementer - used when 'resizing' masks during dynamic resizing
					jQueryFrames.each(function() {					
						id = "ui-layout-mask-"+ (++i);
						jQuery(this).data("layoutMaskID", id); // tag iframe with corresponding maskID
						jQuery('<div id="'+ id +'" class="ui-layout-mask ui-layout-mask-'+ pane +'"/>')
							.css({
								background:	"#fff"
							,	opacity:	"0.001"
							...
							.css(jQuery(this).position()) // top & left -- changed from offset()
			...
			,	drag: function (e, ui) {
					if (!helperClassesSet) { // can only add classes after clone has been added to the DOM
						...
						if (s.isSliding) jQueryPs[pane].css("zIndex", _c.zIndex.pane_sliding);
					}
			...
			*/
			var resizePanes = function (evt, ui, pane, resizingDone) {
				...
				if (resizingDone) {
					...
					jQuery("div.ui-layout-mask").each(function() { this.parentNode.removeChild(this); });
					//jQuery("div.ui-layout-mask").remove(); // TODO: Is this less efficient?
					...
					jQueryFrames.each(function() {
						jQuery("#"+ jQuery(this).data("layoutMaskID")) // get corresponding mask by ID
							.css(jQuery(this).position()) // update top & left
...
,	destroy = function () {
		...
		jQuery(window).unbind("."+ sID);
		jQuery(document).unbind("."+ sID);
		...
		jQueryN	.removeData("layout")
			.removeData("layoutContainer")
			.removeClass(options.containerClass)
		;

		// do NOT reset container CSS if is a 'pane' in an outer-layout - ie, THIS layout is 'nested'
		if (!jQueryN.data("layoutEdge") && jQueryN.data("layoutCSS")) // RESET CSS
			jQueryN.css( jQueryN.data("layoutCSS") ).removeData("layoutCSS");

		// for full-page layouts, also reset the <HTML> CSS
		if (sC.tagName == "BODY" && (jQueryN = jQuery("html")).data("layoutCSS")) // RESET <HTML> CSS
			jQueryN.css( jQueryN.data("layoutCSS") ).removeData("layoutCSS");
...
,	removePane = function (pane, remove, skipResize) {
		if (!isInitialized()) return;
		if (!jQueryPs[pane]) return; // NO SUCH PANE
		var
			jQueryP		= jQueryPs[pane]
		...
		if (!jQueryP || !jQueryP.length) {
			} // pane has already been deleted!
		else if (remove && !jQueryP.data("layoutContainer") && (!jQueryC || !jQueryC.length || !jQueryC.data("layoutContainer")))
			jQueryP.remove();
		else {
			...
			if (!jQueryP.data("layoutContainer"))
				jQueryP.css( jQueryP.data("layoutCSS") ).removeData("layoutCSS");
			// DITTO for the Content elem
			if (jQueryC && jQueryC.length && !jQueryC.data("layoutContainer"))
				jQueryC.css( jQueryC.data("layoutCSS") ).removeData("layoutCSS");
		}

		// REMOVE pane resizer and toggler elements
		if (jQueryT && jQueryT.length) jQueryT.remove();
		if (jQueryR && jQueryR.length) jQueryR.remove();

		// CLEAR all pointers and data
		jQueryPs[pane] = jQueryCs[pane] = jQueryRs[pane] = jQueryTs[pane] = false;
...
,	hide = function (pane, noAnimation) {
		...
		if (!state.initialized || s.isClosed) {
			...
			s.isHidden  = true;
			s.isVisible = false;
			jQueryP.hide(); // no animation when loading page
...
,	toggle = function (pane, slide) {
		...
		if (!isStr(pane)) {
			...
			pane = jQuery(this).data("layoutEdge"); // bound to jQueryR.dblclick
		}
...
,	close = function (pane, force, noAnimation, skipCallback) {
		...
		if (!jQueryP || (!o.closable && !isShowing && !isHiding)) return; // invalid request // (!o.resizable && !o.closable) ???
...
,	setAsClosed = function (pane) {
		...
		if (o.resizable && typeof jQuery.fn.draggable == "function")
			jQueryR
				.draggable("disable")
...
,	open = function (pane, slide, noAnimation, noAlert) {
		...
		if (!jQueryP || (!o.resizable && !o.closable && !isShowing)) return; // invalid request
		...
		if (doFX) { // ANIMATE
			...
			jQueryP.show();	// just show pane and...
			open_2();	// continue
		};
...
,	slideOpen = function (evt_or_pane) {
		...
		,	pane	= evt ? jQuery(this).data("layoutEdge") : evt_or_pane
		,	s		= state[pane]
...
,	lockPaneForFX = function (pane, doLock) {
		var jQueryP = jQueryPs[pane];
		if (doLock) {
			jQueryP.css({ zIndex: _c.zIndex.pane_animate }); // overlay all elements during animation
			if (pane=="south")
				jQueryP.css({ top: sC.insetTop + sC.innerHeight - jQueryP.outerHeight() });
			else if (pane=="east")
				jQueryP.css({ left: sC.insetLeft + sC.innerWidth - jQueryP.outerWidth() });
		}
		else { // animation DONE - RESET CSS
			// TODO: see if this can be deleted. It causes a quick-close when sliding in Chrome
			jQueryP.css({ zIndex: (state[pane].isSliding ? _c.zIndex.pane_sliding : _c.zIndex.pane_normal) });
			if (pane=="south")
			...
			var o = options[pane];
			if (jQuery.layout.browser.msie && o.fxOpacityFix && o.fxName_open != "slide" && jQueryP.css("filter") && jQueryP.css("opacity") == 1)
...
,	bindStopSlidingEvents = function (pane, enable) {
		...
		jQueryR.css("zIndex", enable ? z.pane_sliding : z.resizer_normal);
		...
		jQueryR[action](trigger, slideClose); // base event on resize
		...
		if (!enable)
			...
			jQueryR.attr("title", enable ? o.togglerTip_open : ""); // use Toggler-tip, eg: "Close Pane"
		}
...
,	makePaneFit = function (pane, isOpening, skipCallback, force) {
		...
		if (pane == "center") {
			...
			if (s.size > s.maxSize) // pane is too big - shrink it
			...
			else if (jQueryR && jQueryP.is(":visible")) {
				...
				;
				if (jQuery.layout.cssNum(jQueryR, side) != pos) jQueryR.css( side, pos );
			}
...
,	sizePane = function (pane, size, skipCallback, force) {
		...
		jQueryP.css( dimName, max(1, cssSize(pane, size)) );
...
,	test	= [{
			  	target:		size
			,	attempt:	size
			,	actual:		edge=='width' ? jQueryP.outerWidth() : jQueryP.outerHeight()
			}]
,	lastTest = test[0]
,	thisTest = {}
;
while (lastTest.actual != size) {
	...
	jQueryP.css( edge, cssSize(pane, thisTest.attempt) );

	thisTest.actual = edge=='width' ? jQueryP.outerWidth() : jQueryP.outerHeight()
...
*/
		...
		if (jQueryR && jQueryP.is(":visible")) jQueryR.css( side, size + sC[inset] );
...
,	resizeAll = function () {
		...
		jQuery.extend( state.container, elDims( jQueryN ) ); // UPDATE container dimensions
...
,	resizeNestedLayout = function (pane) {
		...
		if (options[pane].resizeNestedLayout) {
			if (jQueryP.data( d ))
				jQueryP.layout().resizeAll();
			else if (jQueryC && jQueryC.data( d ))
				jQueryC.layout().resizeAll();
		}
	}
...
,	sizeContent = function (panes, remeasure) {
		...
		jQuery.each(panes.split(","), function (idx, pane) {
			...
			function _measure () {
				...
				,	jQueryFs		= jQueryC.nextAll().not(ignore || ':lt(0)') // not :lt(0) = ALL
...
,	sizeHandles = function (panes) {
		...
		jQuery.each(panes.split(","), function (i, pane) {
			...
			if (dir == "horz") { // north/south
				...
				jQueryR.css({
					width:	max(1, cssW(jQueryR, paneLen)) // account for borders & padding
				,	height:	max(0, cssH(jQueryR, spacing)) // ditto
				...
				s.resizerLength = paneLen;
				jQueryR.css({
					height:	max(1, cssH(jQueryR, paneLen)) // account for borders & padding
				,	width:	max(0, cssW(jQueryR, spacing)) // ditto
			...
			if (jQueryT) {
				...
				if (dir == "horz") { // north/south
					...
					,	height:	max(1, cssH(jQueryT, spacing)) // ditto
					...
					jQueryT.children(".content").each(function(){
						...
						jQueryTC.css("marginLeft", Math.floor((width-jQueryTC.outerWidth())/2)); // could be negative
					});
				}
				else { // east/west
					...
					,	width:	max(1, cssW(jQueryT, spacing)) // ditto
					...
					jQueryT.children(".content").each(function(){
						jQueryTC = jQuery(this);
						jQueryTC.css("marginTop", Math.floor((height-jQueryTC.outerHeight())/2)); // could be negative
					});
				}
...
,	enableClosable = function (pane) {
		if (!isInitialized()) return;
		var jQueryT = jQueryTs[pane], o = options[pane];
		if (!jQueryT) return;
		o.closable = true;
...
,	enableSlidable = function (pane) {
		if (!isInitialized()) return;
		var jQueryR = jQueryRs[pane], o = options[pane];
...
,	swapPanes = function (pane1, pane2) {
		...
		function copy (n) { // n = pane
			...
			,	state:		jQuery.extend({}, state[n])
			,	options:	jQuery.extend({}, options[n])
			}
		};
		...
		function move (oPane, pane) {
			...
			options[pane]	= jQuery.extend({}, oPane.options, fx);
			state[pane]		= jQuery.extend({}, oPane.state);
			...
			if (c.dir != _c[oldPane].dir) {
				...
				jQueryRs[pane].css(side, sC[inset] + (state[pane].isVisible ? getPaneSize(pane) : 0));
	...
	*/
	function keyDown (evt) {
		...
		if (CURSOR && options[PANE[code]].enableCursorHotkey) // valid cursor-hotkey
			...
			jQuery.each(_c.borderPanes.split(","), function (i, p) { // loop each pane to check its hotkey
				o = options[p];
				k = o.customHotkey;
 ...
 */
	...
	*/
	function allowOverflow (el) {
		...
		jQuery.each(_c.allPanes.split(","), function(i, p) {
	...
	function resetOverflow (el) {
		...
		if (!s.isSliding && !s.isResizing)
			jQueryP.css("zIndex", _c.zIndex.pane_normal);
	...
	*/
	function addToggleBtn (selector, pane, slide) {
		var jQueryE = getBtn(selector, pane, "toggle");
		if (jQueryE)
	...
	*/
	function addOpenBtn (selector, pane, slide) {
		var jQueryE = getBtn(selector, pane, "open");
		if (jQueryE)
			jQueryE
	...
	*/
	function addCloseBtn (selector, pane) {
		var jQueryE = getBtn(selector, pane, "close");
		if (jQueryE)
			jQueryE
	...
	*/
	function addPinBtn (selector, pane) {
		var jQueryE = getBtn(selector, pane, "pin");
		if (jQueryE) {
			var s = state[pane];
			jQueryE.click(function (evt) {
				setPinState(jQuery(this), pane, (s.isSliding || s.isClosed));
				if (s.isSliding || s.isClosed) open( pane ); // change from sliding to open
				else close( pane ); // slide-closed
				evt.stopPropagation();
			});
			// add up/down pin attributes and classes
			setPinState(jQueryE, pane, (!s.isClosed && !s.isSliding));
	...
	*/
	function syncPinBtns (pane, doPin) {
		jQuery.each(_c[pane].pins, function (i, selector) {
			setPinState(jQuery(selector), pane, doPin);
		});
	};
	...
	*/
	function setPinState (jQueryPin, pane, doPin) {
	...
	*/
	function getCookie (opts) {
		var
			o		= jQuery.extend( {}, options.cookie, opts || {} )
	...
	*/
	function saveCookie (keys, opts) {
		...
		return jQuery.extend({}, state.cookie); // return COPY of state.cookie
	};
	...
	*/
	function loadCookie (opts) {
		...
		if (o) {
			state.cookie = jQuery.extend({}, o); // SET state.cookie
	...
	*/
	function loadState (opts, animate) {
		opts = _transformData(opts);
		jQuery.extend( true, options, opts ); // update layout options
 ...
 */
	...
	// Users retreive Instance of a layout with: jQueryN.layout() OR jQueryN.data("layout")
	...
	,	contents:		jQueryCs				// property - object pointers for ALL Content: content.north, content.center
