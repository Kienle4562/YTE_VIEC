<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
	$lang_text = $core_class->load_module_language('com_search.result.view', $GLOBALS['LANG']);
	//echo $_POST['keyword'];
	...
	$this_keyword = strip_tags($url[0]);
	//	echo $this_keyword;
	/* get total row */
	if($this_keyword == 'tim-kiem')
	{
		$key =  $core_class->_removesigns(strip_tags($url[1]));	
		//echo $key;
		$conditon = "AND `book_product`.`alias` LIKE '%".$key."%'";
	}
	/* config items per page */
	$itemPerPage = 18;
	$totalrow = $categoryProc->get_category_count($conditon);
	/* phan trang */
	//echo $_GET["page"];
	...
	$i = 1;
	$page_title = $categoryProc->get_category_list($key, ' » ', 'title');
	$meta_title = $page_title . ' - ' . $meta_title;
	$meta_title = "Tìm kiếm";
	$pathway_text = '<li class="m_right_8 f_xs_none"><a href="/giay-nu/cp210.html" class="color_default d_inline_m m_right_10">Tìm kiếm</a></li>';
	if ( $totalrow > 0 )
	{
		/* Get category items */
		$result = $categoryProc->get_category($conditon, intval($pager->offset), intval($pager->limit) ); 
?>
<div class="clearfix m_bottom_10">
    Tìm kiếm với từ khóa: <?php echo $key ?>
</div>
                ...
                <figcaption>
                    ...
                    <div class="im_half_container m_bottom_10">
                        ...
                        <div class="half_column d_sm_block w_sm_full d_xs_inline_m w_xs_half_column t_sm_align_c t_xs_align_r d_inline_m t_align_r tr_all animate_fctr with_ie">
                            <ul class="rating_list d_inline_m hr_list tr_all"></ul>
...
<?php }else{ ?><p>Không tìm thấy sản phẩm nào</p><?php } ?>