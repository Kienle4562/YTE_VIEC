<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
    ...
    class process
    {
	    ...
	    function get_menu_edit ($menu_id) {
		    $sql = "SELECT
				      `menu`.`Id`, `menu`.`parent_Id`, `menu`.`title`, `menu`.`link`,
				      `menu`.`link_id`, `menu`.`type`, `menu`.`target`, `menu`.`activated`,
				      `menu`.`order_num`, `menu`.`menu_type_Id`
				    FROM `menu`
				    WHERE `menu`.`Id` = ?
				    ORDER BY order_num;";
	    ...
	    function get_news_from_alias($news_alias)
	    {
		    $sql = "SELECT `news`.`news_id`, `news`.`category_id`, `news`.`title`, `news`.`alias`
				    FROM `news`
				    WHERE `news`.`alias` = ?";
		    $result = $this->dbObj->SqlQueryOutputResult($sql, array($news_alias));
		    return $result;
	    }
        
        function get_product_from_alias($product_alias)
        {
            $sql = "SELECT `book_product`.`Id`, `book_product`.`book_category_id`, `book_product`.`product_name`, `book_product`.`alias`
                    FROM `book_product`
                    WHERE `book_product`.`alias` = ?";
            $result = $this->dbObj->SqlQueryOutputResult($sql, array($product_alias));
            return $result;
        }
	    ...
	    function process_edit_item_menu( $parent_Id, $title, $link, $link_id, $type, $target, $activated, $menu_type_Id, $id ){
		    $sql = "UPDATE `menu` SET
				    `parent_Id` = ?, 
				    `title` = ?, 
				    `link` = ?, 
				    ...
				    `menu_type_Id` = ?
				    WHERE `Id` = ?";
		    $result = $this->dbObj->SqlQueryInputResult($sql, array($parent_Id, $title, $link, $link_id, $type, $target, $activated, $menu_type_Id, $id));
		    if($result > 0){
			    return true;
		    }
		    else return false;
    ...
    */
    ...
    switch($_POST["hidden"])
    {
        ...
        case "submit_com_menu_item_menu_edit";
            $myprocess = new process;
            ...
            if($_POST["task"] == "save"){
                if($myprocess->process_edit_item_menu(
                ...
                if($myprocess->process_edit_item_menu(
                    ...
                    $core_class->_redirect(".?com=com_menu&view=item_menu&task=news.edit&id=".$menu_id);
                    exit();
                } else {
                    $GLOBALS['msg'] = "Đã có lỗi thêm chủ đề, vui lòng làm lại!";
                }
