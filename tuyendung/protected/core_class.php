<?php defined( '_VALID_MOS' ) or die( include("404.php") );
    ...
    class core_class
    {
		...
		function getImage($images){
			...
			if(!empty($images) && strpos($images, "noimage") == false){
				$srcHinhanh = $images;
			}
			return $srcHinhanh;
		}

		// hàm kiểm tra giá trị phần tử trong mảng bị null, null thì trả về true
		function checkNullElementInArray($array)
		{
        ...
        function createCheckBox($table, $content = "")
        {	
			$dbObj = new classDb();
            $sql = "select * FROM `" . $table ."` ORDER BY DISORDER DESC";
        ...
        function getChuyenKhoa($danhmucId){
		    ...
		    $result = $dbObj->SqlQueryOutputResult($sql, array($danhmucId));
		    return $result;
		}
		...
		function getLoaiHinhCongViec(){
			$dbObj = new classDb();
			$sql = "SELECT * FROM mst_loaihinhcongviec ORDER BY DISORDER DESC";
			$result = $dbObj->SqlQueryOutputResult($sql, array());
			return $result;
		}
		...
		function getTrinhDo(){
			$dbObj = new classDb();
			$sql = "SELECT * FROM mst_bangcap ORDER BY DISORDER";
			$result = $dbObj->SqlQueryOutputResult($sql, array());
			return $result;
	    }
		
		function showDataWithArray($stringArray, $table){
			$arrTable = explode("_", $table);
			$sql = "SELECT ten".$arrTable[1]." FROM {$table} WHERE ".$arrTable[1]."_id=";
			$string = "";
			$dbObj = new classDb();
			$array = explode("|", $stringArray);
			foreach($array as $values){
				$result = $dbObj->SqlQueryOutputResult($sql."'".$values."'", array());
				$row = $result->fetch();
				$string .= $row[0].",";
				if(empty($break)){
					$break = "&nbsp;&nbsp;";
		...
		function showDataWithArray2($stringArray, $split = "/"){
			$stringArray = substr($stringArray, 1, strlen($stringArray)-2);
			$arrTable = explode("_", $table);
			$sql = "SELECT * FROM mst_loaiphucloi WHERE loaiphucloi_id=";
			$string = "";
			$dbObj = new classDb();
			$array = explode($split, $stringArray);
			foreach($array as $values){
				$result = $dbObj->SqlQueryOutputResult($sql."'".$values."'", array());
				$row = $result->fetch();
				
				$string .= '<div class="benefit row">';
					$string .= '<div class="benefit-icon col-xs-1">';
						$string .= $row[2];
					$string .= '</div>';
					$string .= '<div class="benefit-name col-xs-11">';
						$string .= $row[1];
					$string .= '</div>';
				$string .= '</div>';
			}
		...
		// Clear ký tự đặc biệt
		function clearSpecialHtml($arrayPost)
		{
		...
		function checkKeyPost($arrayPost){
			foreach($_POST as $key => $value) {
		...
		public function isMobile() {
			...
			return preg_match("/(android|avantgo|blackberry|bolt|boost|cricket|docomo|fone|hiptop|mini|iPhone|iPad|palm|phone|pie|tablet|up\.browser|up\.link|webos|wos)/i", $_SERVER["HTTP_USER_AGENT"]);
		}
		
		public function updateColumnInTable($table, $column, $where) {
			$dbObj = new classDb();
			$sql = "UPDATE {$table} SET {$column} {$where}";
			$dbObj->SqlQueryInputResult($sql, array());
		}
		
		// hàm đếm thời gian
		function time_ago($time, $date_format = 'd/m/Y'){
			$diff = time() - (int)$time;
			if ($diff == 0){  
				 return 'Mới đây';  
			}
			...
			$ago = $value.' '.$intervals[1][0].($value > 1 ? '' : '');  
			...
			if ($ago == '1 ngày'){  
				...
				return date($date_format, $time).' lúc '.date('H:i:s', $time); 
			}
		}
		...
		// Hàm lấy data từ cột
        public function getValueFrom($table, $column, $where)
        {
			$dbObj = new classDb();
			$sql = "SELECT {$column} FROM {$table} WHERE {$where}";
			$result = $dbObj->SqlQueryOutputResult($sql, array());
											...
											$html .= '<div class="col-lg-'.$col.'">';
												...
												if($rowGetColumnInTable["DATA_TYPE"] == "smallint"){
													...
													$html .= '<div class="input-group">';
														$html .= '<input data-toggle="m-popover" data-trigger="focus" data-html="true" data-content="'.$textRequire.' Nhập vào địa chỉ email, độ dài tối đa <code>'.$rowGetColumnInTable["CHARACTER_MAXIMUM_LENGTH"].'</code> ký tự" type="email" id="'.$rowGetColumnInTable["COLUMN_NAME"].'" name="'.$rowGetColumnInTable["COLUMN_NAME"].'" class="form-control m-input" placeholder="'.$rowGetColumnInTable["COLUMN_COMMENT"].'" '.$require.' '.$maxlength.'>';
													...
													$html .= '<div class="input-group">';
														$html .= '<input pattern="[0-9]*" data-toggle="m-popover" data-trigger="focus" data-html="true" data-content="'.$textRequire.' Nhập số điện thoại, độ dài tối đa <code>'.$rowGetColumnInTable["CHARACTER_MAXIMUM_LENGTH"].'</code> ký tự" type="tel" id="'.$rowGetColumnInTable["COLUMN_NAME"].'" name="'.$rowGetColumnInTable["COLUMN_NAME"].'" class="form-control m-input" placeholder="'.$rowGetColumnInTable["COLUMN_COMMENT"].'" '.$require.' '.$maxlength.'>';
														$html .= '<div class="input-group-append">';
															$html .= '<span class="input-group-text">';
													...
													$html .= '<div class="input-group date">';
														$html .= '<input type="text" readonly id="'.$rowGetColumnInTable["COLUMN_NAME"].'" name="'.$rowGetColumnInTable["COLUMN_NAME"].'" class="form-control m-input datepicker" placeholder="'.$rowGetColumnInTable["COLUMN_COMMENT"].'" '.$require.'>';
														$html .= '<div class="input-group-append">';
															$html .= '<span class="input-group-text">';
		...
		function uploadFile()
        {
			if ( 0 < $_FILES['file']['error'] ) {
				...
				$fileName = 'file_upload/career/'. $_REQUEST['profilecv_id'] . $_SESSION['career']['career_id'].".".$fileArr[1];
				$this->resize_image('max', $filePath, $fileName, 200,200);
				unlink($filePath);
				echo $fileName;
			}
		}
        ...
        function createSelectBox($MstTable, $Name, $class="", $where="", $selected = "")
        {
			$dbObj = new classDb();
        ...
        function createSelectBox3($columnId, $required = "", $selected = "", $idname="")
        {	
			$db_name = $GLOBALS['MAP'][$_SERVER['SERVER_NAME']]['db_schema'];
			...
			$html = '<select '.$required.' name="' .$idname. '" class="form-control m-bootstrap-select m_selectpicker" data-live-search="true"> ';
			while($row = $result->fetch()){
				$select = $row[0] == $selected ? "selected" : "";
				$html .= "<option ".$select." value='" .$row[0]. "'>" .$row[1]. "</option>";
			}
		...
		// tạo select box theo id của cột
        function createSelectBoxWithArray($arrayValue, $idname="", $required = "", $selected = "", $class = "form-control m-bootstrap-select m_selectpicker")
        {
			$html = '<select '.$required.' name="' .$idname. '" class="'.$class.'" data-live-search="true"> ';
		...
		// tạo select box theo id của cột
        function createSelectBox4($columnId, $required = "", $selected = "", $idname="")
        {	
			$db_name = $GLOBALS['MAP'][$_SERVER['SERVER_NAME']]['db_schema'];
			$arrayCol = explode("_", $columnId);
			$dbObj = new classDb();
			...
			$html .= "<option value=''>Chọn</option>";
			while($row = $result->fetch()){
				$select = $row[1] == $selected ? "selected" : "";
				$html .= "<option ".$select." value='" .$row[1]. "'>" .$row[1]. "</option>";
			}
			$html .= '</select>';
			return $html;
		}
		...
		// tạo select box theo id của cột
        function createSelectBox5($columnId, $required = "", $selected = "", $idname="")
        {	
			$db_name = $GLOBALS['MAP'][$_SERVER['SERVER_NAME']]['db_schema'];
			$arrayCol = explode("_", $columnId);
			$dbObj = new classDb();
			...
			$html = '<select '.$required.' class="' .$idname. ' form-control m-bootstrap-select m_selectpicker" data-live-search="true"> ';
			$html .= "<option value=''>Chọn</option>";
			while($row = $result->fetch()){
				$select = $row[1] == $selected ? "selected" : "";
				$html .= "<option ".$select." value='" .$row[1]. "'>" .$row[1]. "</option>";
		...
		// tạo select box năm
        function createSelectBoxYear($firstOption="Chọn", $required = "", $values = "", $idname="")
        {	
			$html = '<select '.$required.' class="form-control '.$idname.'" name="'.$idname.'">';
		...
		// tạo select box năm
        function createSelectBoxMonth($firstOption="Chọn", $required = "", $values = "", $idname="")
        {	
			$html = '<select '.$required.' class="form-control '.$idname.'" name="'.$idname.'">';
			$html .= "<option value=''>".$firstOption."</option>";
			for($i=1;$i<=12;$i++){
	...
	public function smtpSendMailCandidate($EmailSubject, $MailContent, $EmailSendToAddress, $fileAttach = "") {	
	...
	function sendMailWithTemplate($titleMail, $mailContent){
		ob_start();
		include("protected/emailcontent.php");
