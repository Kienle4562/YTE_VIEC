<?php defined( '_VALID_MOS' ) or die( include_once("../../404.php") );
    ...
    class process_menu extends modules_models

    {
		function menu($parentid = 0, $group_menu_id, $ext, $max_level, $current_level, &$params)
		{
			...
			{
				if($current_level > 1){
					echo '<ul class="sub_menu r_xs_corners bg_light vr_list tr_all tr_xs_none trf_xs_none bs_xs_none d_xs_none">';
				}			
				...
				while ( $row = $result->fetch( ) )
				{
					...
					if($current_level == 1){
						...
						echo '<a class="color_light relative r_xs_corners" href="', $link, '" target="', $row['target'], '">';																						
						echo $row["title"];
						...
						{
							$this->menu( $row["Id"], $group_menu_id, $ext, $max_level, $current_level + 1, $params );
						}											
						echo '</li>';
					} else {
						echo '<li>';								
